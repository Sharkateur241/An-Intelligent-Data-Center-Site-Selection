...
class EnhancedDataCenterAnalysis:
    """增强版数据中心选址分析"""
    ...
    async def analyze_comprehensive(self, lat: float, lon: float, city_name: str) -> Dict[str, Any]:
        """综合数据中心选址分析"""
        print(f"🔍 开始综合数据中心选址分析: {city_name}")
        print("=" * 60)
        
        analysis_result = {
            "location": {"latitude": lat, "longitude": lon, "city_name": city_name},
    ...
    async def _analyze_energy_supply(self, lat: float, lon: float, city_name: str) -> Dict[str, Any]:
        """分析能源供应与成本"""
        try:
            from backend.services.energy_assessment import EnergyAssessmentService
            from backend.services.energy_ai_analysis import EnergyAIAnalysisService
            
            energy_service = EnergyAssessmentService()
            ai_service = EnergyAIAnalysisService()
            ...
            # AI增强分析
            ai_result = await ai_service.analyze_energy_resources_ai(energy_data)
...
async def main():
    ...
    analyzer = EnhancedDataCenterAnalysis()
    ...
    for location in test_locations:
        ...
        print(f"\n📊 {location['name']} 分析结果:")
        print(f"   总体得分: {result['overall_score']:.1f}/10")
        print(f"   建议: {result['recommendation']}")
        
        if result['key_strengths']:
            print(f"   优势: {', '.join(result['key_strengths'])}")
